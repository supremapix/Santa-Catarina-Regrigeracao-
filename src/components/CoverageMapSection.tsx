...
export const CoverageMapSection: React.FC = () => {
  ...
  return (
    <section className="bg-slate-900 py-16 lg:py-24 text-white border-b border-slate-800" id="cobertura">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Filter Controls Bar */}
        <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
          {/* Ring Filter Buttons */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredCities.map((city: CityLocalSEO) => (
            <div
              key={city.slug}
              className="bg-slate-950 border border-slate-800/90 rounded-2xl p-4 hover:border-cyan-500/50 transition-all flex flex-col justify-between group"
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${
                    city.ring === 1 ? 'bg-emerald-950 text-emerald-400 border-emerald-800' :
                    city.ring === 2 ? 'bg-cyan-950 text-cyan-400 border-cyan-800' :
                ...
                <div className="space-y-1 text-[11px] text-slate-400 mb-3 border-t border-slate-900 pt-2">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1">
                      <Navigation className="w-3 h-3 text-cyan-400" /> Distância Penha:
                    </span>
                    <span className="font-semibold text-slate-200">{city.distanceKm} km</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3 text-emerald-400" /> Tempo Estimado:
                    </span>
                    <span className="font-semibold text-slate-200">~{city.estimatedMinutes} min</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-slate-900 flex items-center justify-between gap-2">
                ...
                <a
                  ...
                  className="px-2.5 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-[10px]"
                >
                  Chamar
                </a>
              </div>
